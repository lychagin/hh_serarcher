...
4. **Единицы запросов, с паузой между ними**: `robots.txt` каждого
   origin, ОДНА страница листинга и ОДНА страница вакансии; id берётся из
   первого ответа, а не зашивается в тест. hh.ru отвечает на
...
def test_vacancy_page_still_exposes_job_posting(client: PoliteClient, listing_html: str) -> None:
    ...
    # Сторожем за дрейфом служит агрегатный `SalaryBlockStats` («ни на одной
    # странице прогона»), а не одна случайная вакансия.
    assert 'data-qa="vacancy-salary' in page or extract_salary(page) is None
