...
или `partial` (частичная потеря работы), но прогон, в котором источник не отдал
ни одной страницы листинга, — это `failed`: иначе полная потеря сети выглядела
бы для healthcheck успехом.
