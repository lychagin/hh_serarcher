...
Утверждение прежней редакции («секции `User-agent: *` нет») **неверно**: живой `robots.txt`
hh.ru такую секцию содержит, и в ней есть `Disallow: *?*` — запрет любого URL с
query-строкой, то есть всего RSS-поиска. Разрешены `/vacancies/{slug}` и постраничный
листинг (`Allow: /vacancies/*?page=`, `Allow: /vacancies/*?*&page=`) — на них и переезжает
discovery. Пользовательское соглашение hh может отдельно ограничивать автоматизированный
сбор — принятый режим работы это учитывает.
...
- соблюдение `robots.txt` в рантайме по RFC 9309 — собственным матчером, а не
  `urllib.robotparser`: та не поддерживает подстановки (`*`, `$`) и не смотрит на
  query-строку, из-за чего запреты hh.ru были для неё невидимы;
- проверка `robots.txt` того хоста, на который реально идёт запрос, включая каждый хоп
  редиректа (hh.ru уводит региональные вакансии на hh.kz и rabota.by);
