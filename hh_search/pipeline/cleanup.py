...
`sinks/base.py` — константа живёт при протоколе `Sink`, а не в конкретном
приёмнике, и прямой зависимости от РЕАЛИЗАЦИИ `TelegramSink` ради этого
числа больше нет. Транзитивный импорт всего пакета `sinks` (а с ним и
`telegram_sink.py`, и `httpx`) при этом никуда не делся — `sinks/__init__.py`
жадно импортирует `TelegramClient`/`TelegramSink` ради фабрики `build_sinks`,
и это свойство пакета `sinks`, существовавшее и до переноса этой константы,
а не что-то, что решает данный перенос. Импортом константы, а не
