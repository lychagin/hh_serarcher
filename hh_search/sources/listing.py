...
попадает RSS-поиск (`/search/vacancy/rss?text=...`) и общий поиск
(`/search/vacancy?text=...`) — на первом стоял шаг discovery до раунда
2026-07-28. Проверено живой загрузкой 2026-07-28, фикстура —
`tests/fixtures/robots_hh.txt`.

Разрешённых форм ровно три, все проверены матчером на живых правилах:
    ...
    /vacancies/{slug}?work_format=F&page=N   — `Allow: /vacancies/*?*&page=`: это
                                                правило разрешает ЛЮБЫЕ параметры при
                                                наличии `&page=`, и hh.ru написал его
                                                сознательно (проверено матчером на
                                                живой фикстуре правил). Без `&page=`
                                                тот же параметр запрещён:
                                                `?work_format=F` без `&page=` попадает
                                                под `Disallow: *?*`
                                                (`test_work_format_without_page_would_be_forbidden`).

Третья форма — второй поток discovery, `build_listing_url` c `query.work_format`.
Параметр `area` в ней не используется — не потому, что запрещён (он прошёл бы под
то же правило `Allow: /vacancies/*?*&page=`), а потому что **игнорируется hh.ru**:
регион на пути `/vacancies/{slug}` задаёт не параметр, а поддомен, на который
hh.ru редиректит по IP. Замер 2026-07-30: `?area=2&page=0` (Петербург), запрошенный
с нижегородского хоста, дал нижегородскую выдачу — 0 пересечений из 20 с
петербургской и 11 из 20 с нижегородской. Параметр `work_format`, в отличие от
`area`, работает: 6 из 6 вакансий из `?work_format=REMOTE&page=0`, проверенных по
странице, содержат REMOTE (тот же замер). Подробности —
`docs/superpowers/specs/2026-07-30-region-and-work-format-design.md` §0 и
`docs/superpowers/specs/2026-07-27-hh-autosearch-design.md` §3.2, §3.5 (поправка
2026-07-30, отменяющая прежнюю редакцию обоих утверждений).

`QuerySpec.slug` сужен регуляркой `^[a-z0-9][a-z0-9-]*$` и отвергает query-строку,
дот-сегменты и весь прочий URL-синтаксис в самом slug на старте, до первого
запроса (см. `config/models.py`); `work_format` приезжает отдельным типизированным
полем `QuerySpec.work_format`, а не через slug. Второй половиной той же гарантии
служит `http.normalize_url()`: robots проверяется по НОРМАЛИЗОВАННОМУ URL, поэтому
построить здесь строку, которая проверится как одна, а уйдёт в сеть как другая,
...
def build_listing_url(query: QuerySpec, page: int = 0) -> str:
    ...
    первой странице. `?page=0` тоже был бы законен: `Allow: /vacancies/*?page=`
    длиннее запрета `Disallow: *?*` и потому побеждает его. Но это защита,
    купленная лишним query-параметром там, где он не нужен, а голый путь не
    совпадает вообще ни с одним правилом — ни с запретом, ни с разрешением.
