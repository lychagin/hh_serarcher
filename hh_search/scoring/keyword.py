...
class KeywordScorer:
    ...
    def _region_penalty(self, area: str | None, work_formats: frozenset[WorkFormat]) -> float:
        ...
        Правила стоят в том порядке, в каком их называет §6 спеки: домашний
        регион не штрафуется ни при каком формате — иначе штраф убивал бы офис
        в родном городе; иначе REMOTE среди форматов снимает штраф — вакансия
        может предлагать сразу несколько форматов, и одного REMOTE
        достаточно; иначе штраф. Порядок здесь пока НЕ наблюдаем: все три
        ветки — короткие замыкания на `return 0.0`, поэтому перестановка
        первых двух не меняет ни одного вердикта (проверено мутацией). Он
        станет значим ровно тогда, когда домашняя ветка перестанет возвращать
        ноль — например, если у неё появится бонус вместо отсутствия штрафа.
        Неизвестные регион (`area` пуст, состоит из
