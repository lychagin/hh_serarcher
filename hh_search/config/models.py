...
class LimitsConfig(Base):
    ...
    Умолчания выбраны так, чтобы штатный режим их не заметил: сумма `pages`
    образцового конфига далеко не заполняет `listing_pages_per_run`, а отчёт
    отдаёт десятки вакансий при потолке в 500 строк. Само число запросов шага
    discovery здесь не называется: оно обязано протухать от любой правки
    `config.example/queries.yaml` и уже протухло в четырёх копиях разом.
    Единственное место, где оно живёт, — §4.1 спеки, и там его сторожит
...
class LlmConfig(Base):
    ...
    # Умолчания ВЫКЛЮЧЕНЫ, хотя образец конфига их включает. Разница
    # адресована обновлению: у уже работающей установки секции `llm` в
    # `app.yaml` нет вовсе, и умолчание `true` означало бы, что сервис
    # после обновления сам начинает ходить в сеть за моделью, которой на
    # той машине может не быть, — и пишет ERROR каждый прогон. Новая
    # возможность включается решением владельца, а не фактом обновления.
    semantic: bool = False
    facts: bool = False
...
class ProfileConfig(Base):
    ...
    # Связное описание владельца ПРОЗОЙ — для локальной модели и только
    # для неё: ключевой скоринг его не читает. Заведено замером
    # 2026-08-26 (§0.10 спеки `2026-08-26-local-llm-design.md`): один и
    # тот же вход с описанием профиля фразой давал вакансии «Technical
    # Team Lead (C# / Python)» оценку 35, а с перечнем сигналов — 75.
    # Различающая способность живёт в прозе, и перечнем её не заменить.
    #
    # Причина, судя по всему, в том, чего в сигналах нет ПО ПОСТРОЕНИЮ:
    # стажа и того, кем владелец себя видит. `signals` описывают, ЧТО
    # искать, а не КТО ищет.
    #
    # Пусто — мнение у модели не спрашивается вовсе. Молчание честнее
    # неразличающего числа: без прозы она отвечает 65-85 почти всем, и
    # такое число выглядит оценкой, ею не являясь.
