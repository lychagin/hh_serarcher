...
1. Не читается ОЦЕНКА (`score`/`score_detail`). Оценка вычисляется
   ...
   трогаем никогда.
2. Не читается что-либо ещё: `id`, `title`, `cluster`, `primary_query`,
   `company`, `area`, `published_at`, `salary_*` или само описание.
   Заголовка, кластера и запроса на странице вакансии нет вовсе, а всё
   остальное она уже отдала — описание записано, и повторно её не
   скачивает ни одна из трёх выборок (`pending_enrichment` берёт только
   `description IS NULL`). Источника восстановления, таким образом, не
   существует, и самовосстановление здесь бессмысленно. Лечение — сразу
   терминальный статус, одна запись в лог, ни одна колонка не обнуляется
   (испорченные значения и есть улики).
