...
class TelegramSink:
    ...
    4. `send_document` — если падает, файл уже опубликован: следующий
       ...
       останется БЕЗ отметки о доставке (`_SENT_SUFFIX`). `maintain`
       довозит документы всех дней окна без отметки КАЖДЫЙ прогон, вне
       зависимости от того, есть ли свежие вакансии (T-2), не трогая
       сообщение (item 1): оно уже ушло, повтор был бы дублем.
    ...
    def _redeliver(self, previous: Sequence[tuple[date, str, set[str]]]) -> None:
        ...
        Застрявший день опознаётся по ФАКТУ: файл дня есть, а отметки о
        его доставке нет, то есть тот прогон отправил сообщение и упал
        именно на `send_document`. Прежняя редакция опознавала его по
        совпадению ссылок («вакансия нашлась только в файле предыдущих
        суток») и принимала за него две здоровые ситуации сразу — упавшего
        соседа-приёмника и повторный `report --since`, — то есть слала в
        канал дубль документа.
        ...
        Права на каталог отчётов проверяются ДО отправки (находка C1).
        Отметка ставится ПОСЛЕ `send_document`, и недоступный на запись
        каталог (`:ro`, `chmod 0o500`, съехавший owner после рестарта
        контейнера) иначе давал бы `PermissionError` на `marker.touch()`
        уже ПОСЛЕ ушедшего документа: отметка не появляется, а `maintain`
        зовётся КАЖДЫЙ прогон и отказ не красит статус (`maintain_sinks`
        глушит его сознательно, T-2) — тот же документ уезжал бы в канал
        снова и снова, при статусе `ok`. Пробная запись — тот же приём,
        что черновик в `emit`: дешёвая, ДО сети, а не после.
        """
        stuck = [
            (day, content)
            for day, content, _ in previous
            if content and not self._marker(self._day_file(day)).exists()
        ]
        if not stuck:
            return
        self._ensure_reports_dir_is_writable()
        for day, content in stuck:
            self._client.send_document(
                self._day_file(day).name, content.encode("utf-8"), f"Отчёт за {day:%Y-%m-%d}"
            )
            self._marker(self._day_file(day)).touch()

    def _ensure_reports_dir_is_writable(self) -> None:
        """Пробная запись в каталог отчётов — до сети (находка C1).

        `tempfile.mkstemp` создаёт файл сразу и падает, если каталог
    ...
    def _sweep_orphaned_drafts(self) -> None:
        ...
        Безопасно ровно потому, что `maintain` вызывается под общим замком
        ...
        черновик ЭТОГО прогона создаётся позже, изнутри `emit`, самим
        `_draft`, и сюда не попадает.

        Отказ самой уборки глушится: черновик — мусор, а не данные, и
        недоступный на запись каталог отчётов не имеет права ронять
        `maintain`, который без уборки прошёл бы дальше как ни в чём не
        бывало (каталог `0o500` с осиротевшим `.part` внутри давал новый
        класс отказа, лечимый только руками). Не убранный черновик
